...
client.once('ready', () => { 
    ...
    const channel = client.channels.cache.get(YOUR_CHANNEL);
    setInterval(async () => {
            try {
                const response = await fetch(`https://opendata.cwa.gov.tw/api/v1/rest/datastore/E-A0015-001?Authorization="YOUR_API_KEY"&format=JSON`);
